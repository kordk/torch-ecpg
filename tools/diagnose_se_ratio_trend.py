#!/usr/bin/env python3
"""Reports how boot_se_ratio behaves against |mt_t| within each region.

Read-only over the parquet written by annotate_bootstrap_concordance.py. It
sets no threshold, raises no flag, removes no row, and writes nothing to its
input: it measures so that a threshold decision can later be made from observed
distributions rather than assumed ones. Statistics cover the rows carrying a
finite boot_se_ratio and a finite mt_t; rows whose region is null or is not in
CANONICAL_REGIONS are reported under an explicit unassigned bucket rather than
dropped.
"""
import argparse
import json
...
def main():
    ...
    for region_name, group in df_scored.groupby("assigned_region"):
        ...
        if n_scored < args.min_region_n:
            ...
            if np.isfinite(rho_val):
                ...
                reg_dict["spearman_rho_omitted_reason"] = (
                    "spearman statistic is not finite, which occurs when one "
                    "input is constant"
                )
    ...
    err = sys.stderr
    print(f"n_rows_read: {n_rows_read}", file=err)
    print(f"n_scored: {total_scored}", file=err)
    print(f"params: {out['params']}", file=err)
    print(file=err)
    print(f"{'region':<15} {'n_scored':<10} {'median_r':<10} {'mad_r':<10} {'t_min':<10} {'t_max':<10} {'rho':<10} {'ci'}", file=err)
    for k, v in out_regions.items():
        ...
        print(f"{k:<15} {v['n_scored']:<10} {v['median_se_ratio']:<10.3f} {v['mad_se_ratio']:<10.3f} {v['t_abs_min']:<10.3f} {v['t_abs_max']:<10.3f} {rho_str:<10} {ci_str}", file=err)
        if v["bins"]:
            print(f"  Bins:", file=err)
            for b in v["bins"]:
                if b["n"] > 0:
                    print(f"    bin {b['bin']}: n={b['n']}, t_abs in [{b['t_abs_lo']:.3f}, {b['t_abs_hi']:.3f}], med_r={b['median_se_ratio']:.3f}", file=err)
                else:
                    print(f"    bin {b['bin']}: n=0", file=err)
        print(file=err)
