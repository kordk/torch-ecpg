...
GPU_UNDERUTIL_RATIO = 0.5
GPU_UNDERUTIL_SM_PCT = 50
LOW_RAM_GB = 2.0
HIGH_CPU_PCT = 90
...
class Logger(PassAsKwarg):
    ...
    def __init__(
        ...
        *,
        ...
        if parent is not None:
            ...
            if os.environ.get('TECPG_PROFILE') == '1':
                self.carry_data['profile'] = True
    ...
    def resource_check(self) -> Dict[str, Any]:
        """
        Returns a snapshot of current system and GPU resources.
        """
        process = psutil.Process(os.getpid())
        ram_rss = process.memory_info().rss / 1024 ** 3
        ...
        return {
            'cpu_percent': cpu_pct,
            'ram_rss_gb': ram_rss,
            'ram_avail_gb': sys_vm.available / 1024 ** 3,
            'swap_used_gb': sys_swap.used / 1024 ** 3,
            'gpu_alloc_gb': gpu_alloc,
            'gpu_res_gb': gpu_res,
            'gpu_max_alloc_gb': gpu_max_alloc,
        }

    def print_startup_banner(self, **kwargs) -> None:
        """Prints system/GPU info once at start"""
        self.info('--- STARTUP BANNER ---')
