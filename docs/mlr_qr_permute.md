...
> **Status: under active development.** The **Phase-1 trans pipeline is complete** — `qr_permute` produces real per-pair p-values for the **trans** stratum (empirical p with a GPD tail extension), on a finalized, seeded, provenance-stamped output schema. **It is not yet ready for inference.** The Phase-3a diagnostics now run on real data through the **canonical 7-way region taxonomy** (`assignRegionToEcpg_parquet.py`: `TRANS`, `DISTAL5`, `CIS5`, `PROMOTER`, `GENEBODY`, `CIS3`, `DISTAL3`), so the genuine near-gene regions (`CIS5`/`PROMOTER`/`GENEBODY`/`CIS3`) are no longer conflated with distal same-chromosome pairs. The **reservoir-first per-region run is complete on full GTP** (§5): in the mostly-null bulk the permutation and analytic p-values agree to within ~0.3% with `lambda_trans ≈ 1.006`, and — decisively — **both well-powered distal strata calibrate identically to `TRANS`** (`DISTAL5`/`DISTAL3` bulk departures within ~1e-5, `divergent_regions = []`). Two consequences: the same-chromosome distal space behaves exactly like trans, so a *per-chromosome structural* divergence is ruled out by evidence; and the trans analytic null is validated across the whole same-chromosome space, not just inter-chromosomal pairs. The near-gene window itself, however, is **too sparse in a uniform reservoir to evaluate** — pooled near-gene bulk fell below the coverage floor (`insufficient_near_gene_coverage`), which is the anticipated branch that routes to a **cis-window enrichment master**. That enrichment path is now **built and merged** (a chr-prefix-native cis write-all map via the `tecpg/chrom.py` canonicalizer, `tools/build_gene_anchored_master.py` to assemble the near-gene pairs with the reservoir's trans/distal pairs, and a `pipelinePermute.sh --cis-enrich` entrance). Its first run exposed **two latent correctness bugs in the base mapping tool's cis/distal window predicate** — an `int8` overflow that silently collapsed a requested ±1 Mb window to an effective ~±64 bp, and a bound-ordering error that made a symmetric window *empty* for every negative-strand gene — both since fixed (§4). Neither touches `region='all'` mapping (which skips the predicate entirely), so mainline catalogs are unaffected; but any prior windowed `--cis`/`--distal` map ran on a collapsed window. The **corrected enrichment run, and the deciding cis-window verdict read per dataset, are the pending items.** The cis stratum still has no dedicated null, so **every reported pair is currently scored against the trans-global null**, and `perm_mt_p` is not wired into the downstream FDR pipeline (Phase 4). Do **not** use `perm_mt_p` for inference until this notice is updated. See [§5 Implementation status](#5-implementation-status).
...
- **Phase 3 — evaluation / diagnostics** *(partially delivered)*. A standalone read-only script. Its arms divide by what the persisted output can support (§3.7):
  ...
  **Enrichment run (full GTP) — first attempt and correction.** The first `pipelinePermute.sh --cis-enrich` run validated the wrapper end-to-end (chr-prefix-native region filtration, map → `mergeOutputs` → assemble staging) but produced only **2,741** cis pairs from a requested ±1 Mb window, and failed at the assemble step. Two independent defects, both now fixed: the assembly tool did not promote `mt_id`/`gt_id` from the map parquet's named index (the map writes them as an index; the rest of the codebase promotes them with an explicit guard), and — the substantive one — the windowed-map predicate's `int8` overflow and negative-strand bound reversal (§4), which together meant the "±1 Mb" window was really ~±64 bp on positive-strand genes and empty on negative-strand ones. With those corrected, the enrichment re-run is in flight; the near-gene counts it produces are the coverage gate, and the resulting 7-way verdict is the decision point for Phase 2.
...
- **Partly resolved — stratify-or-not, cis window.** The reservoir-first per-region run (full GTP, §5) settled everything except the cis window itself: `TRANS` and both distal strata calibrate together (`divergent_regions = []`), which **rules out the per-chromosome-structural branch** — the near-gene window is now the only open question, and post-enrichment the routing is binary (near-gene calibrated ⇒ single global null carries and Phase 2 is unnecessary; near-gene diverges ⇒ per-gene Beta). The near-gene regions were too sparse in the uniform reservoir to evaluate (`insufficient_near_gene_coverage`), so the **cis-window enrichment master** is now *triggered* rather than held in reserve, and is built (§4): a chr-prefix-native cis write-all map → `build_gene_anchored_master.py` (assemble with the reservoir's trans/distal pairs) → re-annotate → permute → per-region eval, wrapped by `pipelinePermute.sh --cis-enrich`. The remaining open item is the **cis-window verdict from that enrichment run, read per dataset** (GTP, then MESA, then the oncology cohort). The first enrichment attempt was invalidated by two windowed-map defects (§4, §5) rather than by anything about the calibration; with those fixed the corrected run is in flight, and only its `Counts by Region` line can say whether the near-gene family finally clears `MIN_REGION_BULK_N`. **Coverage is the master's universe regardless of how this resolves.**
