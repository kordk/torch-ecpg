...
> **Status: under active development.** The **Phase-1 trans pipeline is complete** — `qr_permute` produces real per-pair p-values for the **trans** stratum (empirical p with a GPD tail extension), on a finalized, seeded, provenance-stamped output schema. **It is not yet ready for inference.** The Phase-3a diagnostics have been run on real data (GTP subset, reservoir calibration master — §5): in the mostly-null bulk the permutation and analytic p-values agree to within ~1% and `lambda_trans ≈ 1.0`, so the **trans analytic null looks well-calibrated**. The stratify diagnostic now consumes the project's **canonical 7-way region taxonomy** (`assignRegionToEcpg_parquet.py`: `TRANS`, `DISTAL5`, `CIS5`, `PROMOTER`, `GENEBODY`, `CIS3`, `DISTAL3`) instead of a same-chromosome heuristic, so the genuine near-gene regions (`CIS5`/`PROMOTER`/`GENEBODY`/`CIS3`) are no longer conflated with distal same-chromosome pairs — the mislabel that made the earlier reservoir verdict *suspicious not settled*. The **reservoir-first per-region confirmation** (annotate the reservoir, permute, evaluate per region — one `pipelinePermute.sh` invocation) is pending (§5, §8); a cis-window enrichment is held in reserve only if the reservoir's near-gene coverage proves too thin. The cis stratum still has no dedicated null, so **every reported pair is currently scored against the trans-global null**, and `perm_mt_p` is not wired into the downstream FDR pipeline (Phase 4). Do **not** use `perm_mt_p` for inference until this notice is updated. See [§5 Implementation status](#5-implementation-status).
...
- **Phase 3 — evaluation / diagnostics** *(partially delivered)*. A standalone read-only script. Its arms divide by what the persisted output can support (§3.7):
  ...
  **3a run status (GTP subset).** The calibration diagnostic needs a master that spans the full p-value range (a mostly-null *bulk*); a threshold-filtered master (e.g. `bootstrap_merged.parquet`, significant pairs only) yields an empty bulk, a `lambda` of ~27 that is a filter artifact rather than inflation, and a skipped verdict. The right calibration master is the mapping's **`sample_reservoir.csv`** — a uniform sample taken *before* p-value filtration (`--reservoir-count`), carrying `mt_t` over the full range — converted to parquet and consumed like any other master (`pipelinePermute.sh --reservoir` automates the CSV→parquet conversion, the region-annotation, the two-step run, and the summary; `tools/summarize_permute.py` renders the report into a text digest plus figures). On that master the **trans** bulk calibration is near-perfect (median `|log10(p_perm/p_ana)|` ≈ 0.003, `lambda_trans` ≈ 1.007) and the trans null's bulk calibration is validated.
  ...
  That first run, however, split strata with the eval's legacy same-chromosome heuristic, whose "cis" was ~98 % distal — so its `single_global_null_adequate` verdict was *suspicious not settled* for the cis window. The **7-way per-region path is now in place** (the eval consumes the canonical `region` column, and `pipelinePermute.sh` annotates the reservoir before permute — §4), so the deciding run is the **reservoir-first per-region evaluation**: one `pipelinePermute.sh --reservoir` invocation produces the per-region verdict directly. The `assignRegionToEcpg` `Counts by Region` line is the free **coverage gate** — if the near-gene regions (`CIS5`/`PROMOTER`/`GENEBODY`/`CIS3`) are adequately populated in the reservoir, the per-region verdict stands on its own; only if they come back `insufficient_data` is a cis-window enrichment master warranted (§8). The eval thresholds (`BULK_LO`, `TOLERANCE`, `MIN_REGION_BULK_N`) remain provisional placeholders to be re-set from full GTP/MESA.
...
- **Open — stratify-or-not, cis window (reservoir-first).** The first reservoir run returned `single_global_null_adequate`, but under the eval's legacy same-chromosome split, whose "cis" was ~98 % distal — so it was *suspicious not settled*. The fix is the region *labeling*, not the input: the full-map reservoir already samples every region, and the canonical 7-way taxonomy (§3.4) cleanly separates the near-gene family from distal. The deciding run is therefore **reservoir-first per-region** — annotate the reservoir, permute, evaluate per region (one `pipelinePermute.sh --reservoir`) — with three outcomes: near-gene calibrated like `TRANS` ⇒ Phase 2 unnecessary and a single global null carries (and the trans analytic+FDR fork below opens); near-gene diverges while distal does not ⇒ a genuine cis-window effect and Phase 2 warranted; near-gene and distal diverge together ⇒ a same-chromosome structural effect calling for a per-chromosome null rather than per-gene Beta. A **cis-window enrichment master** (a windowed map feeding the near-gene regions, assembled with the reservoir's trans pairs) is held in reserve *only if* the reservoir's `Counts by Region` shows the near-gene regions are underpowered (`insufficient_near_gene_coverage`) — evidence-contingent, not predetermined. **Coverage is the master's universe regardless of how this resolves.**
