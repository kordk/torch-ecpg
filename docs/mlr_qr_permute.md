...
- **Phase 4 — mainline annotation** *(two of three chunks merged)*. The verdict changes what this phase has to be. Because every GTP stratum calibrates, `p_permute` for a calibrated stratum **is** the validated analytic p — which is the array `precise_mt_p` already contains — so there is no per-pair permutation to merge and no brute-force scan. Two decisions bound the work. First, `p_permute` and `fdr_permute` are added as **standard columns** rather than omitted where redundant, so the schema is stable across cohorts and downstream consumers can rely on them for a dataset whose verdict differs. Second, the ordering forbids doing it in `[7/9]`: that stage runs inside `pipeline.sh`, before any permutation exists, and repopulating it would invalidate the bootstrap candidate list and everything `pipelinePost.sh` consumes. The annotation is therefore a **new final stage, `pipelinePermute.sh [5/5]`**, built in three chunks.

  *Chunk A — merged (`7dfa516`).* `tools/summarizeOutput_parquet.py` gains `--p-column` and `--fdr-column`, defaulting to `precise_mt_p`/`fdr_est` so the existing `pipeline.sh:305` invocation is unchanged. Three guards accompany it: an explicitly named p-column that is absent **fails closed** rather than falling back to the t-statistic (which would write analytic values under a permutation name); naming an existing column as `--fdr-column` is refused, since a second invocation would otherwise overwrite the analytic FDR in place; and a row whose source p is null now yields a **null** FDR rather than `1.0`, keeping *not assessed* distinguishable from *assessed and not significant*.

  *Chunk B — merged (`b11ee1e`).* `tools/annotate_permute_p.py` reads the catalog and `eval_permute_report.json` and, keyed on `region`, copies `precise_mt_p` into a new `p_permute` column for the licensed strata, leaving it null elsewhere. **Licensing is keyed on the per-region status *and* the `divergent_regions` list, not on status alone.** `eval_permute.py` records a divergent region with `status: 'ok'` and marks divergence only by list membership — there is no `'divergent'` status — so a status-only predicate would license precisely the strata the verdict excluded. The predicate is `{R : status ∈ (ok, reference)} − set(divergent_regions)`. Rows are left null for a divergent region, a region below the bulk floor, a region absent from the report, and a null region label. The write is additive and refuses to overwrite an existing column; an unrecognized region label fails closed rather than being silently nulled, and a mid-stream failure removes its partial output rather than finalizing a truncated parquet.

  *Chunk C — pending.* The wrapper stage that invokes both, with `--input`/`--output` so it can be applied to `summarized.parquet` and to `bootstrap_merged.parquet` (the latter is what `pipelinePost.sh` reads). The write is additive; `mt_p`, `precise_mt_p` and `fdr_est` are never touched. **No selector is changed** — `createBootstrapList.py` keeps ranking on `precise_mt_p`, so the candidate list and all of `pipelinePost.sh` stay bit-identical; promoting `p_permute` to a ranking axis remains a deliberate one-line decision for later.
