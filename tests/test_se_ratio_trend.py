...
def _build_frame():
    ...
    # Unscored TRANS rows, |t| strictly below every scored row and drawn from
    # no random stream, so a statistic computed over all rows differs from one
    # computed over the scored rows by construction rather than by chance.
    unscored = pd.DataFrame({
        "region": pd.Series(["TRANS"] * N_UNSCORED, dtype=object),
        "mt_t": np.linspace(T_UNSCORED_LO, T_UNSCORED_HI, N_UNSCORED),
        "boot_se_ratio": np.full(N_UNSCORED, np.nan),
    })
...
def test_t_range_is_over_scored_rows_only(run_result):
    ...
    assert summary["regions"]["TRANS"]["t_abs_min"] >= T_LO
...
# --- P0-A2: strict JSON, degenerate regions, stream hygiene, import ---------


def test_summary_is_strict_json(run_result):
    """NaN and Infinity are not JSON; withheld values must serialize as null."""
    _, _, _, d = run_result

    def _reject(name):
        raise AssertionError(f"summary carries a non-JSON constant: {name}")

    with open(d / "summary.json") as fh:
        json.load(fh, parse_constant=_reject)


def test_constant_region_yields_null_rho_not_nan(tmp_path):
    """A region with no variation in r must withhold rho with a reason, not emit NaN."""
    rng = np.random.default_rng(5)
    n = 400
    df = pd.DataFrame({
        "region": ["CIS5"] * n + ["TRANS"] * n,
        "mt_t": np.concatenate([rng.uniform(T_LO, T_HI, n), rng.uniform(T_LO, T_HI, n)]),
        "boot_se_ratio": np.concatenate([np.full(n, 1.07), rng.normal(1.07, 0.05, n)]),
    })
    inp = tmp_path / "concordance.parquet"
    pq.write_table(pa.Table.from_pandas(df, preserve_index=False), inp)
    out_json = tmp_path / "summary.json"
    ...
    reg = summary["regions"]["CIS5"]
    assert reg["spearman_rho"] is None
    assert reg["spearman_rho_omitted_reason"] is not None
    assert summary["regions"]["TRANS"]["spearman_rho"] is not None


def test_json_to_stdout_is_parseable_without_summary_path(tmp_path):
    """With -s omitted, stdout carries the JSON alone; the table goes elsewhere."""
    inp = tmp_path / "concordance.parquet"
    _write_fixture(inp)
    proc = subprocess.run(
        [sys.executable, TOOL, "-i", str(inp), "--min-region-n", str(MIN_REGION_N)],
        capture_output=True, text=True)
    assert proc.returncode == 0, proc.stderr
    parsed = json.loads(proc.stdout)
    assert parsed["n_scored"] > 0
    assert "region" in proc.stderr


def test_eval_permute_import_is_required(tmp_path):
    """The region taxonomy has one source; a failed import must abort, not fall back."""
    import shutil
    isolated = tmp_path / "isolated"
    isolated.mkdir()
    shutil.copy(TOOL, isolated / "diagnose_se_ratio_trend.py")
    inp = tmp_path / "concordance.parquet"
    _write_fixture(inp)
    proc = subprocess.run(
        [sys.executable, str(isolated / "diagnose_se_ratio_trend.py"),
         "-i", str(inp), "-s", str(tmp_path / "summary.json")],
        capture_output=True, text=True)
    assert proc.returncode != 0
    assert "eval_permute" in proc.stderr
